...
public class HillClimbing{
    ...
    public void gerarAleatoriamente(int[][] tabuleiro, int[] estado) {
        //iteração através de índices de coluna
        for (int i = 0; i < 8; i++) {
            // Obtenção de um índice de linha aleatória
            estado[i] = random();

            // Coloca uma rainha na posição obtida no tabuleiro.
            tabuleiro[estado[i]][i] = 1;
        }
    }
    // Uma função que imprime o "quadro" da matriz.
    public void exibirTabuleiroDeXadrez(int[][] tabuleiro){
    ...
    // Preenchendo a matriz "tabuleiro" com os valores "valor"
    public void preencherTabuleiro(int[][] tabuleiro, int valor) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++){
                tabuleiro[i][j] = valor;
            }
    ...
    // Calcula o valor objetivo do Estado (as rainhas atacam entre elas)
    // Função de avaliação heurística - o nome das regras que fazem atacar umas às outras.
    public int hDoParDeAtaque(int[][] tabuleiro, int[] estado)
    {

        /*
        Para cada rainha em uma coluna, verificamos se outras rainhas
        se alinham com a nossa rainha atual e se encontrarmos alguma,
        qualquer, então incrementamos a contagem da variável de ataque.
        */
    ...
    public void hillClimbing(int[][] tabuleiro, int[] estado){
        ...
        // inicializa a matriz e o estado vizinhos com a placa e o estado da matriz atual
        copia(estadoVizinho, estado);
        gerandoTabuleiro(quadroVizinho, estadoVizinho);
        ...
        do
        {
			...
			*/
            ...
            if (Arrays.equals(estado, estadoVizinho)) {
                ...
                /*
				 Se o estado vizinho e o estado real forem iguais,
                 então não há vizinho ideal e, conseqüentemente,
